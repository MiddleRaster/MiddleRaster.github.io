...
4. Third, we have missing functions, like ```std::to_string()```. You can either write a forwarding function by hand (good for function templates), or use the MOCKABLE_FUNCTION (good for C APIs in the global namespace), or use the ```Callable``` template.
