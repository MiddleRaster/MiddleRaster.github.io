...
The Roman Numerals kata:  write a function to convert an integer to its Roman numeral representation.

We'll need either a gazillion little tests or one big one.  Let's go with the one big one.
    ...
    public:
        TEST_METHOD(ToRoman)
        {
            struct testData
            {
                int number;
                const char* numeral;
            } td[] = // Source: http://www2.inetdirect.net/~charta/Roman_numerals.html.
            {
                {1   , "I" },
                {2   , "II" },
                {3   , "III" },
